...
function generateOrderId() {
  ...
  orderCounter++;
  return `ORD-${date}-${time}\nToken: ${String(orderCounter).padStart(3,"0")}`;
}
...
async function sendOrderNotifications(orderId, dateTime, customer, items) {
  ...
  const receipt = `🍕 *Order Receipt*\n\nOrder ID: ${orderId}\nDate & Time: ${dateTime}\n\n👤 Customer: ${customer.name}\n📱 Phone: ${customer.phone}\n🏠 Address: ${customer.address}\n\n🧾 Items:\n${itemDetails}*Grand Total: ₹${totalPrice}*\n\nThank you for ordering! Visit again!`;

  const notify = `📢 *New Order Notification*\n\nOrder ID: ${orderId}\nDate & Time: ${dateTime}\n\n👤 Customer: ${customer.name}\n📱 Phone: ${customer.phone}\n🏠 Address: ${customer.address}\n\n🧾 Items:\n${itemDetails}*Grand Total: ₹${totalPrice}*\n\nPlease prepare ASAP! 🍕`;

  const payloads = [
    { messaging_product: "whatsapp", to: toCustomer, type: "text", text: { body: receipt } },
    { messaging_product: "whatsapp", to: toChef, type: "text", text: { body: notify } },
    { messaging_product: "whatsapp", to: toManager, type: "text", text: { body: notify } }
  ];
...
function autoSendAfterPayment() { processAndSendWhatsApp();}
